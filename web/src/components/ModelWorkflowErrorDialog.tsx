...
export interface ModelWorkflowErrorDialogProps {
  groupId: string | null
  open: boolean
  onClose: () => void
  entityKind: string
...
export function ModelWorkflowErrorDialog({
  groupId,
  open,
  onClose,
  entityKind,
  entityLabel,
  fetchWorkflowErrors,
}: ModelWorkflowErrorDialogProps) {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [details, setDetails] = useState<ModelWorkflowErrorResponse | null>(null)
  ...
  useEffect(() => {
    ...
    void fetchWorkflowErrors(groupId)
      .then((response) => {
        if (!cancelled) {
          setDetails(response)
        }
  ...
  }, [fetchWorkflowErrors, groupId, open])
  ...
  return (
    <Dialog
      ...
      aria-labelledby="model-workflow-error-title"
      fullWidth
      maxWidth="md"
      slotProps={{
        backdrop: {
    ...
    >
      <DialogTitle id="model-workflow-error-title" sx={{ pb: 1 }}>
        <Stack spacing={1}>
          ...
          <Typography color="text.secondary" variant="body2">
            {entityLabel ?? (groupId ? `${entityKind} ${groupId}` : entityKind)}
          </Typography>
        </Stack>
      </DialogTitle>
