...
class TradeRecord(BaseModel):
    datetime: str | None = Field(default=None, description="Timestamp when the trade was closed")
    entry_bar_index: int | None = Field(default=None, description="Bar index when the trade was opened")
    exit_bar_index: int | None = Field(default=None, description="Bar index when the trade was closed")
    size: float = Field(description="Signed trade size in shares or contracts")
    price: float = Field(description="Execution price at trade close")
    value: float = Field(description="Notional value of the closing trade")
    pnl: float = Field(description="Gross profit or loss before commission")
    pnlcomm: float = Field(description="Net profit or loss after commission")
    reason: str | None = Field(default=None, description="Exit reason or closing trigger for the trade")
    entry_datetime: str | None = Field(default=None, description="Timestamp when the trade was opened")
    hold_minutes: float | None = Field(default=None, description="Time the trade was held, measured in minutes")
    hold_bars: int | None = Field(default=None, description="Number of bars the trade remained open")
    regime_label: str | None = Field(default=None, description="Market regime associated with the trade exit")
...
class ReportSummaryRecord(RunSummaryBase):
    run_id: str = Field(description="Unique run identifier within the backtest report")
    name: str | None = Field(default=None, description="Optional display name for the run")
    status: Literal["success", "failed"] = Field(description="Terminal status for the run")
    strategy: str = Field(description="Strategy name that produced the run")
    symbol: str | None = Field(default=None, description="Symbol traded by the run, if any")
    data_source: str = Field(description="Resolved data source used by the run")
    start_value: float | None = Field(default=None, description="Starting portfolio value")
    end_value: float | None = Field(default=None, description="Ending portfolio value")
    return_pct: float | None = Field(default=None, description="Total return in percent")
    max_drawdown_pct: float | None = Field(default=None, description="Maximum drawdown in percent")
    sharpe_ratio: float | None = Field(default=None, description="Sharpe ratio for the run")
    total_trades: int = Field(default=0, description="Total number of trades executed")
    won_trades: int = Field(default=0, description="Number of winning trades")
    lost_trades: int = Field(default=0, description="Number of losing trades")
