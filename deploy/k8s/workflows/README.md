...
Every workflow includes a `print-payload` step at launch time. It runs in parallel with the first work step so it can log a copy-pasteable command without gating the rest of the workflow. For backtests, it logs a `curl` command for `POST /backtests/argo` using the workflow parameters. Override the API target when port-forwarding locally:
